...
namespace Face.Web.Core.FaceAI
{
    ...
    public class FaceLiveness
    {
        ...
        /// 测试单目RGB静默活体检测（传入图片文件路径)
        /// </summary>
        /// <param name="file">需要检测的图片，小于10M，图片的本地文件路径地址</param>
        /// <returns></returns>
        public string RGB_LivenessCheck(string file)
        {
            // 传入图片文件绝对路径
            IntPtr ptr = rgb_liveness_check(file);
            string buf = Marshal.PtrToStringAnsi(ptr);
            return buf;
        }
        ...
        /// 测试单目RGB静默活体检测（传入图片文件二进制buffer) 
        /// </summary>
        /// <param name="file">需要检测的图片，二级制格式，小于10M</param>
        /// <returns></returns>
        public string RGB_LivenessCheckByBuffer(string file)
        {
            // 传入图片文件绝对路径
        ...
        /// </summary>
        /// <param name="rgb_buf">可见光图片的二进制图片信息（rgb），须小于10M</param>
        /// <param name="ir_buf">近红外图片的二进制图片信息（ir），须小于10M</param>
        /// <returns></returns>
        public string RGB_IR_LivenessCheckByBuffer(byte[] rgb_buf, byte[] ir_buf)
        {
            IntPtr ptr =  rgb_ir_liveness_check_by_buf(rgb_buf, rgb_buf.Length, ir_buf, ir_buf.Length);
            string buf = Marshal.PtrToStringAnsi(ptr);
            Console.WriteLine(" rgb_ir_liveness_check_by_buf result is " + buf);
            return buf;
        }
